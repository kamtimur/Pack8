...
u32 pack8(const s32* RESTR in_diff, EventMark mark, s32 num_bits, s32* RESTR out_pkcd) //TODO(romanm): поменять возвращаемое значение на знаковое во всей цепочке вызовов
{
	u32 ch0 = in_diff[0];
	u32 ch1 = in_diff[1];
	u32 ch2 = in_diff[2];
	...
	case 4:
		...
		*out_pkcd++ = mark_num | (0b1000) | (ch0 << 8) | (ch1 << 12) | (ch2 << 16) | (ch3 << 20) | (ch4 << 24) | (ch5 << 28);
		*out_pkcd = ch6 | (ch7 << 4);
		return 2;
	case 5:
...
//u32 pack(const s32* RESTR in_diff, EventMark mark, s32* RESTR out_pkcd)
//{
//	for (s32 i = 0; i< 8; i++)
//	{
//		Diff[i] = rg = src[i] - Prev[i]; // внимание, порядок вычитания снова изменен
//		Prev[i] = src[i];
//		rg = size(rg);
//		if (frm < rg) frm = rg;			//
//	};
